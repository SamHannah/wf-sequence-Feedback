# runs a simulation of a word frequency sequence effect experiment with pre-determined accuracy feedback 
# for first 20 trials,  at some level of L, F, SF and dp1,
...
sequence<- function(SF1, dp1, L, A, N_subjects, fback){
  ...
  Frequency<- 0    
  N_el = 100; N_items = 40
  N_seqSegment = 2                       # cutting quartet means into halves (N_seqSegment = 2), or quartiles or deciles 
  ...
  pos<- round(PTrials*fback,0)
  FeedBack<- c(rep(0, times=PTrials-pos),rep(1, times=pos))
  ...
  quartMeans<- apply(summaryQ, c(1,2), qMean)  # each row of quartMeans holds means for one test quarter across test items, 
  quartError<- apply(summaryQ, c(1,2), se)     # with columns holding means for a test item across test quaters
  ...
  qMeans<- data.frame(Half=factor(N_seqSegment*N_types, levels = seq(1:N_seqSegment)), 
